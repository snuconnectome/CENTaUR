#!/bin/bash
# Train Qwen2.5-32B QLoRA using NGC PyTorch (Simplified)
# Uses ngc-python wrapper

set -e

echo "=========================================="
echo "Qwen2.5-32B QLoRA Training (NGC PyTorch)"
echo "=========================================="
echo ""
...
echo "$SUDO_PASSWORD" | sudo -S docker run --rm \
  ...
  " 2>&1 | tee logs/qwen25-32b-qlora_ngc_$(date +%Y%m%d_%H%M%S).log
